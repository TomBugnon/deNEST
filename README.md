...
- We call __model__ a NEST model (whether of a neuron or synapse) with a
  specific name and set of parameters. All models are copied from their
  respective "NEST model" using `nest.Copy()` with their specific parameters.
  NEST models can be either imported from external modules or built-in.
...
- We call __layer__ a NEST topological layer. A layer is akin to a grid of a
  ...
  - Layers can be of the type `InputLayer` when composed of generators. Input
    ...
    reflect the forecoming input stimulus (see 'session' paragraph)
...
- We call __connection_model__ a set of parameters describing a certain type of
  projection between node populations. The following types of connection_models
  are handled so far:
  ...
  - _connections from file_: Connections between individual units loaded from a
    dump from a previous simulation. Arbitrary units to units connections are
    handled with this type of connections.
...
- We call __session__ a period of simulation of a network with specific
  ...
  - One can change the state of the network before each session by:
    ...
    - One can change the state of the input layers of the network to mimic the
      ...
      by specifying an __input__ for that session. An input stimulus is an array
...
1. __Initialize network__:
  1. Initialize the `Network` object representing a NEST network: derive the
     dependent parameters from the independent parameters given in parameter.
...
tree are combined in a ChainMap _before_ leaves inherit the parameters of their
parent nodes.
...
- ``"path" parameters``: Parameters from the main simulation parameter file,
  passed to the main simulation parameters passed during a direct `run()` call
  or a CLI package call.
...
- `simulation` (leaf): Contains the simulation parameters.
  - The following parameters are recognized:
    ...
    - `input_dir` (str or None): Path to the input. Can be overwritten by the
      ...
      paragraph for details on how the input is loaded for each session.
      (default from `user_config.py`)
...
- `sessions` (subtree): Contains the parameters for each session and the list of
  ...
  - `time_per_frame` (float): Number of milliseconds during which each 'frame'
    of the input movie is shown to the network. (default 1.)
...
- `network` (subtree): Should contain the following subtrees:
  - `layers` (subtree): Defines layer parameters. Each leaf is a layer. The
    ...
    - `edge_wrap` (bool): Whether the layer is wrapped in NEST. (default false)
    ...
    - `weight_gain`: Scaling factor for the weight of connections of which the
      considered layer is the _source_. (default 1.)
    - `scale_kernels_masks_to_extent`: Whether the kernels and masks of
      ...
      extent. (default True).
        - __Important__: Please note that by default the kernel and masks are
  ...
  - `connection_models` (subtree): Defines the connection models. Each leaf is a
    ...
    - The other parameters define the topological or rescaled (topological)
      ...
      connections.
    - __Important__: For each connection model, if the
  ...
  - `populations` (subtree): Defines the populations (= models within a specific
    ...
    - __NB__: Make sure that no population is missing compared to the 'layer'
      parameters.
