...
UNIT_CHANGES = [
    {"change_type": "multiplicative", "population": None, "params": {"V_m": 1.10}},
    {
        "change_type": "constant",
        "population": "iaf_psc_alpha",
        "params": {"V_m": 0.0, "E_L": 0.0},
    },
]
...
def test_unit_changes(base_layer, unit_changes):
    ...
    for key, value in params.items():
        if unit_changes["change_type"] == "constant":
            assert all(
                [
                    nest.GetStatus((gid,), key)[0] == value
                    for gid in base_layer.gids(population=population)
                ]
            )
            ...
            assert all(
                [
                    nest.GetStatus((gid,), key)[0] == value * current_values[key][gid]
                    for gid in base_layer.gids(population=population)
                ]
            )
        else:
